...
def test_default_suite_matches_baseline():
    ...
    assert result["matched"] == result["n"] == 10
...
def test_threat_family_coverage():
    ...
    assert set(result["families"]) == {
        "abuse", "adaptive", "collusion", "honeypot", "prompt_injection",
        "scam"}
    assert all(result["families"].values())          # every family defended
    assert result["families_defended"] == 6
    assert "Threat families defended: 6/6" in suite.render_markdown(result)
